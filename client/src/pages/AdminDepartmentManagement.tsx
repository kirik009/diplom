...
export default function AdminDepartmentManagement() {
  ...
  const { data: departments, isLoading: departmentsLoading, error : departmentsError} = useQuery<(Department & { facultyName: string })[]>({
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button variant="ghost" size="icon" className="h-8 w-8" 
                                  onClick={() => setLocation(`/admin/edit-user/${department.id}`)}>
      ...
      <Dialog open={isConfirmDialogOpen} onOpenChange={setIsConfirmDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Вы уверены, что хотите удалить эту кафедру? Это действие нельзя отменить.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
